...
struct clock {
	...
	int *timelinesfd; 		/* Timeline File Descriptor */
...
struct clock *clock_create(enum clock_type type, struct config *config,
			   const char *phc_device, int timelineid, int *timelinesfd)
{
	enum timestamp_type timestamping =
	...
	STAILQ_FOREACH(iface, &config->interfaces, list) {
		if (iface->ts_info.valid &&
		    ...
		    /* Modified for the QoT Stack -> Use SW Timestamping if HW Timestamping not supported*/
			if (config_set_int(config, "time_stamping", TS_SOFTWARE))
			{
				return NULL;
			}
		}
	...
	/* QoT Stack Related Timeline Metadata */
	c->timelineid   = timelineid;
	c->timelinesfd  = timelinesfd; 
	c->tml_clkid    = FD_TO_CLOCKID(timelinesfd[0]);
...
enum servo_state clock_synchronize(struct clock *c, tmv_t ingress, tmv_t origin)
{
	double adj, weight;
	enum servo_state state = SERVO_UNLOCKED;
	struct timespec ingress_tml;             // Added for the QoT Stack
	tmv_t ingress_tml_tmv;				     // Added for the QoT Stack

	c->ingress_ts = ingress;

	/* QoT Stack Project received timestamp to timeline reference */
	if(clock_project_timeline(c->tml_clkid, ingress, &ingress_tml)){
		pr_warning("[T%i]: timeline projection failed", c->timelineid);
		return SERVO_UNLOCKED;
	}

	/* Convert timeline timestamps to tmv */
	ingress_tml_tmv = timespec_to_tmv(ingress_tml);

	/* Enter in Values to the ts system*/
	tsproc_down_ts(c->tsproc, origin, ingress_tml_tmv);

	/* Compute offset */
	if (tsproc_update_offset(c->tsproc, &c->master_offset, &weight))
		return state;

	/* Replace timestamps with original timestamps */
	tsproc_down_ts(c->tsproc, origin, ingress);

	if (clock_utc_correct(c, ingress_tml_tmv))
	...
	adj = servo_sample(c->servo, tmv_to_nanoseconds(c->master_offset),
			   tmv_to_nanoseconds(ingress_tml_tmv), weight, &state);
	c->servo_state = state;
	...
	if (c->stats.max_count > 1) {
		...
		pr_info("[T%i]: master offset %10" PRId64 " s%d freq %+7.0f "
			"path delay %9" PRId64, c->timelineid,
