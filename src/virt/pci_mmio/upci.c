/*************************************************************************
 ...
 Copyright (C) 2017 Sandeep D'souza, Carnegie Mellon University
 ...
 (Some modifications for the QoT Stack for Linux)
...
int upci_scan_bus(void)
{
	...
	if (f == NULL) {
		errmsg(__func__, "opening '%s': %s", DEVICES_FILE,
		       strerror(errno));
		goto cleanup1;
	}
	lineptr = NULL;
	...
	while (1) {
		if (num_devs == MAX_DEVICES) {
			errmsg(__func__, "devices list full (%d entries)",
			       num_devs);
		...
		n = sscanf(lineptr,
			   ...
			   &dev->p.base_addr[0], &dev->p.base_addr[1],
			   &dev->p.base_addr[2], &dev->p.base_addr[3],
			   &dev->p.base_addr[4], &dev->p.base_addr[5],
			   &dev->p.rom_base_addr, &dev->p.size[0],
			   &dev->p.size[1], &dev->p.size[2], &dev->p.size[3],
			   &dev->p.size[4], &dev->p.size[5], &dev->p.rom_size);
		if (n != 17) {
			errmsg(__func__, "only %d items on %s line %d", n,
			       DEVICES_FILE, linenum);
			goto cleanup3;
		}
		...
		snprintf(dev_fname, 255, DEVICE_FILE, dev->p.bus, dev->p.dev,
			 dev->p.func);
		fd = open(dev_fname, O_RDONLY);
		if (fd < 0) {
			errmsg(__func__, "opening '%s': %s", dev_fname,
			       strerror(errno));
			goto cleanup3;
		}
		n = read(fd, &cfg, sizeof(struct cfg_info));
		if (n != sizeof(struct cfg_info)) {
			errmsg(__func__, "reading '%s': %s", dev_fname,
			       strerror(errno));
			goto cleanup4;
		}
		close(fd);
...
int upci_print_device_info(int devnum)
{
	...
	printf("Vendor/Device/SSVendor/SSDevice: %04x/%04x/%04x/%04x\n",
	       dev->vendor_id, dev->device_id, dev->ss_vendor_id,
	       dev->ss_device_id);
	for (n = 0; n < 6; n++) {
		if (dev->size[n] > 0) {
...
static int incr_io_usage(void)
{
	...
	if (ioaccess == 0) {
		...
		if (seteuid(0) != 0) {
			errmsg(__func__,
			       "need root privilges (or setuid root)");
			return -1;
		}
		/* do it */
		retval = iopl(3);
		...
		if (retval < 0 || iopl(3) < 0) {
			errmsg(__func__, "opening I/O ports: %s",
			       strerror(eno));
			return -1;
		}
	}
...
static int incr_mem_usage(void)
{
	...
	if (memaccess == 0) {
		...
		/* check result */
		if (memfd < 0) {
			errmsg(__func__, "can't open /dev/mem: %s",
			       strerror(eno));
			return -1;
		}
	}
...
int upci_open_region(int devnum, int region_num)
{
	...
	if (reg->type == UPCI_REG_IO) {
		...
		if (reg->mapped_ptr == MAP_FAILED) {
			reg->mapped_ptr = NULL;
			errmsg(__func__,
			       "can't map /dev/mem for device %d, region %d: %s",
...
__u8 upci_read_u8(int rd, __u32 offset)
{
	...
	if ((rd < 0) || (rd >= MAX_REGIONS) ||
	    ((reg = regions[rd]) == NULL) || (offset > reg->size))
		return 0;
	/* get the data */
	if (reg->type == UPCI_REG_IO) {
		/* region is IO */
...
// Addition for the QoT Stack --> Return a void pointer to the data
void* upci_read_data(int rd, __u32 offset)
{
	struct region_info *reg;
	void *ptr;
	/* test for out of range, not mapped, or offset beyond end of region */
	if ((rd < 0) || (rd >= MAX_REGIONS) || ((reg = regions[rd]) == NULL) || (offset > reg->size))
		return 0;
	/* get the data */
	ptr = (void*)(reg->mapped_ptr + offset);
	return ptr;
}
